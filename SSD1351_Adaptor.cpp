...
void SSD1351_Adaptor::set_write_pos_(uint8_t x, uint8_t y)
{
	uint8_t bytes[] = {
		2, 0x15, x,
		2, 0x75, y,
...
/* Fill certain region */
void SSD1351_Adaptor::fill_rect(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t colour)
{
	select();
...
void SSD1351_Adaptor::hline(uint16_t x, uint16_t y, uint16_t len, uint16_t colour)
{
	select();
	set_write_pos_(x, y);
...
/* Draw vertical line */
void SSD1351_Adaptor::vline(uint16_t x, uint16_t y, uint16_t len, uint16_t colour)
{
	select();
...
void SSD1351_Adaptor::write_begin(uint16_t x, uint16_t y, uint16_t w, uint16_t h, bool col_order)
{
	select();
	if (col_order)
		set_write_order_(true);
...
/* Write pixels */
void SSD1351_Adaptor::write_pixels(uint16_t const* pix_buff, int len)
{
...
/* Write pixels bitmap */
void SSD1351_Adaptor::write_pixels_bm(uint8_t const* pix_bm, int len, uint16_t colours[2])
{
	uint8_t bit = 0;
	uint8_t pixels = 0;
	for (; len > 0; --len) {		
